...
function ResourcePage({ config, navigationContent = null }) {
  ...
  const resolvedToolbarContent = isAuditLogsPage
    ? tableToolbarContent
    : isProductStylePage && hasSelectedProductStyleRows
  ...
  return (
    <div className={`resource-center__page resource-center__page--${config.key} ${isAuditLogsPage ? 'resource-center__page--users' : ''} ${isProductStylePage && !isAuditLogsPage ? 'resource-center__page--product-style products-page' : ''}`}>
      {isAuditLogsPage ? (
        <header className="resource-center__inventory-header resource-center__users-header" aria-label="Audit Logs summary">
      ...
      <div className={`card ${isAuditLogsPage ? 'resource-center__inventory-table-card' : isProductStylePage ? 'products-table-card resource-center__products-table-card' : ''} ${isSubCategoriesPage ? 'resource-center__table-card' : ''} ${isInventoryCompactPage ? 'resource-center__inventory-table-card' : ''} ${!isProductStylePage && isAuditLogsPage ? 'resource-center__audit-table-card' : ''} ${isNotificationsPage ? 'resource-center__notifications-table-card' : ''} ${isInvoicesPage ? 'resource-center__invoices-table-card' : ''}`}>
        <DataTable
          ...
          columnStorageKey={isAuditLogsPage ? 'ims.auditLogs.visibleColumns.usersLayout.v2' : isProductStylePage ? `ims.${config.key}.visibleColumns.productsStyle.v1` : isSubCategoriesPage ? 'ims.subCategories.visibleColumns.warehouseParity.v1' : isNotificationsPage ? 'ims.notifications.visibleColumns.v2' : isInvoicesPage ? 'ims.invoices.visibleColumns.v2' : ''}
          ...
          fitExplicitColumnsToContainer={isAuditLogsPage}
        />
      </div>
