...
function getPurchaseOrderReceiptStatusClass(status) {
  const normalizedStatus = String(status || '').trim().toLowerCase()
  if (['received', 'completed', 'paid', 'reconciled', 'delivered', 'approved', 'success'].includes(normalizedStatus)) return 'badge-green'
  if (['ordered', 'pending', 'partial', 'partially received', 'partially_received', 'partially paid', 'partially_paid', 'sent'].includes(normalizedStatus)) return 'badge-amber'
  return 'badge-red'
}
...
function buildReceiptSvg({ payment, purchaseOrder, invoice, metrics, generatedBy, isSupplier = true }) {
  const paymentStatus = getPaymentStatusMeta(payment.status).label
  const referenceStatus = isSupplier
    ? formatPurchaseOrderStatus(purchaseOrder?.status || payment.poStatus)
    : getInvoiceStatusMeta(payment.invoiceStatus).label
  const receiptNumber = paymentNumberFrom(payment)
  const referenceNumber = isSupplier
    ? getPurchaseOrderNumber(payment, purchaseOrder)
    : getInvoiceNumber(payment, invoice)
  const generatedOn = getGeneratedTimestamp()
  const companyLines = getReceiptCompanyLines()
  const referenceDate = formatReceiptDate(isSupplier ? (purchaseOrder?.orderDate || payment.poDate || payment.paymentDate) : (invoice?.invoiceDate || payment.invoiceDate || payment.paymentDate))
  const remainingBalance = Number(metrics.outstandingAfter || 0)

  const documentTag = isSupplier ? 'SUPPLIER PAYMENT RECEIPT' : 'CUSTOMER PAYMENT RECEIPT'
  const partyTypeLabel = isSupplier ? 'Supplier' : 'Customer'
  const referenceTypeLabel = isSupplier ? 'Purchase Order' : 'Invoice Number'
  const partyName = formatPersonName(payment.partyName || (isSupplier ? (purchaseOrder?.supplierName || 'Supplier') : 'Customer'))
  const totalLabel = isSupplier ? 'Purchase Order Total' : 'Invoice Total Amount'
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="${PDF_CANVAS_WIDTH}" height="${PDF_CANVAS_HEIGHT}" viewBox="0 0 ${PDF_CANVAS_WIDTH} ${PDF_CANVAS_HEIGHT}">
      <defs>
        <linearGradient id="headerGrad" x1="0%" y1="0%" x2="100%" y2="100%">
          ...
          <stop offset="100%" stop-color="${isSupplier ? '#0f766e' : '#2563eb'}"/>
        </linearGradient>
      </defs>
      ...
      <style>
        ...
        .meta-tag { fill: ${isSupplier ? '#2dd4bf' : '#60a5fa'}; font: 800 11px Arial, sans-serif; letter-spacing: 1.2px; }
      ...
      <rect x="490" y="66" width="232" height="26" rx="13" fill="${isSupplier ? 'rgba(45,212,191,0.18)' : 'rgba(96,165,250,0.18)'}" stroke="${isSupplier ? 'rgba(45,212,191,0.4)' : 'rgba(96,165,250,0.4)'}"/>
      ...
      <text class="section-title" x="52" y="232">${escapeXml(partyTypeLabel.toUpperCase())} &amp; ${isSupplier ? 'ORDER' : 'INVOICE'} DETAILS</text>
      ...
      <text class="card-val" x="242" y="292">${escapeXml(referenceNumber)}</text>

      <text class="card-label" x="414" y="268">${isSupplier ? 'Order Date' : 'Invoice Date'}</text>
      <text class="card-val" x="414" y="292">${escapeXml(referenceDate)}</text>

      <text class="card-label" x="586" y="268">Status</text>
      <rect class="${isSupplier ? getPurchaseOrderReceiptStatusClass(referenceStatus) : getInvoiceReceiptStatusClass(referenceStatus)}" x="582" y="278" width="124" height="26" rx="13"/>
      <text class="${isSupplier ? getPurchaseOrderReceiptStatusClass(referenceStatus).replace('badge-', 'badge-txt-') : getInvoiceReceiptStatusClass(referenceStatus).replace('badge-', 'badge-txt-')}" x="644" y="295" text-anchor="middle">${escapeXml(referenceStatus)}</text>
...
async function downloadPaymentReceiptPdf({ payment, purchaseOrder, allPayments, generatedBy }) {
  if (typeof document === 'undefined' || typeof Image === 'undefined') {
    throw new Error('PDF generation is only available in the browser.')
  }

  const metrics = getSupplierReceiptAllocationMetrics(payment, purchaseOrder, allPayments)
  const svgMarkup = buildReceiptSvg({ payment, purchaseOrder, metrics, generatedBy })
  ...
  anchor.href = url
  anchor.download = `Supplier-Payment-Receipt-${receiptNumber}.pdf`
  anchor.click()
  URL.revokeObjectURL(url)
}
...
function getPurchaseOrderNumber(payment, purchaseOrder) {
  return payment.poNumber
    || purchaseOrder?.poNumber
    || (payment.poId ? `PO-${String(payment.poId).padStart(3, '0')}` : 'Not applied')
}

function formatPurchaseOrderStatus(value) {
  const status = String(value || 'Ordered').trim().replace(/[_-]+/g, ' ')
...
function isSamePurchaseOrderPayment(payment, selectedPayment) {
  const selectedPoId = String(selectedPayment?.poId || '').trim()
  const paymentPoId = String(payment?.poId || '').trim()
  return Boolean(selectedPoId && paymentPoId && selectedPoId === paymentPoId)
}

function getSupplierReceiptAllocationMetrics(payment, purchaseOrder, allPayments = []) {
  const purchaseOrderTotal = Number(payment.poAmount || purchaseOrder?.totalAmount || 0)
  const selectedSortKey = getPaymentSortKey(payment)
  const selectedRowId = getPaymentRowId(payment)
  const previousPayments = allPayments
    .filter((candidate) => (
      getPaymentRowId(candidate) !== selectedRowId
      && isEffectiveReceiptPayment(candidate)
      && isSamePurchaseOrderPayment(candidate, payment)
      && getPaymentSortKey(candidate) < selectedSortKey
    ))
    .reduce((sum, candidate) => sum + Number(candidate.amount || 0), 0)
  const currentPayment = Number(payment.amount || 0)
  ...
  return {
    invoiceTotal: purchaseOrderTotal,
    previousPayments,
    outstandingBefore: Math.max(0, purchaseOrderTotal - previousPayments),
    outstandingAfter: Math.max(0, purchaseOrderTotal - previousPayments - currentPayment),
  }
}

function getPurchaseOrderPaymentHistory(payment, allPayments = []) {
  return allPayments
    .filter((candidate) => isEffectiveReceiptPayment(candidate) && isSamePurchaseOrderPayment(candidate, payment))
    .sort((firstPayment, secondPayment) => getPaymentSortKey(firstPayment).localeCompare(getPaymentSortKey(secondPayment)))
}
...
function PaymentDetailsDrawer({ payment, purchaseOrder, allPayments = [], onClose, onPrint, onDownloadReceipt, onExport, onOpenPurchaseOrder }) {
  ...
  const purchaseOrderStatus = formatPurchaseOrderStatus(purchaseOrder?.status || activePayment.poStatus)
  const metrics = getSupplierReceiptAllocationMetrics(activePayment, purchaseOrder, allPayments)
  const paymentHistory = getPurchaseOrderPaymentHistory(activePayment, allPayments)
  const balanceTone = getBalanceTone(metrics, '')
  const progressPercent = getPaymentProgressPercent(metrics, activePayment)
  const purchaseOrderTotal = Number(metrics.invoiceTotal || 0)
  const paidAmount = Math.max(0, Math.min(purchaseOrderTotal, purchaseOrderTotal - Number(metrics.outstandingAfter || 0)))
  const timelineTotalPaid = paymentHistory.reduce((sum, historyPayment) => sum + Number(historyPayment.amount || 0), 0)
  const activePaymentRowId = getPaymentRowId(activePayment)
  const purchaseOrderNumber = getPurchaseOrderNumber(activePayment, purchaseOrder)
  const referenceNumber = activePayment.referenceNumber || ''
  const paymentSummaryRows = [
    { label: 'Purchase Order Total', value: formatCurrency(metrics.invoiceTotal) },
      ...
      <aside className="payment-drawer__panel">
        ...
        <section className="payment-drawer__summary-card" aria-label="Payment summary">
          ...
          <div className="payment-drawer__summary-customer">
            <span>Supplier</span>
            <strong>{formatPersonName(activePayment.partyName || purchaseOrder?.supplierName || purchaseOrder?.supplier || 'Supplier')}</strong>
          </div>
        </section>
        ...
        <div className="payment-drawer__content">
          <section className="payment-drawer__finance-card" aria-label="Financial summary">
            <div className="payment-drawer__section-header">
              ...
              <StatusBadge type="info">{purchaseOrderStatus}</StatusBadge>
            </div>
          ...
          <section className="payment-drawer__section payment-drawer__section--compact">
            ...
            <dl className="payment-info-inline">
              <DetailItem
                ...
                value={
                  purchaseOrderNumber && purchaseOrderNumber !== 'Not Linked' ? (
                    <span className="payment-detail-copy-value">
                      <button
                        type="button"
                        className="payment-drawer__invoice-link"
                        onClick={() => onOpenPurchaseOrder?.(activePayment, purchaseOrder)}
                      >
                        {purchaseOrderNumber}
                      </button>
                      <button
                        type="button"
                        className={`payment-copy-button ${copiedKey === 'po-number' ? 'is-copied' : ''}`.trim()}
                        onClick={() => handleCopy(purchaseOrderNumber, 'po-number')}
                        aria-label="Copy purchase order number"
                        title={copiedKey === 'po-number' ? 'Copied' : 'Copy purchase order number'}
                      >
                        <Copy size={13} />
                      </button>
                    </span>
          ...
          <section className="payment-drawer__section payment-drawer__timeline-card">
            <div className="payment-drawer__timeline-header">
              ...
              <strong>Total Paid: {formatCurrency(timelineTotalPaid)}</strong>
            </div>
    ...
    >
      <form className="payment-form payment-edit-form" onSubmit={handleSubmit}>
        <div className="payment-form__readonly-grid">
          ...
          <DetailItem label="Supplier" value={formatPersonName(payment.partyName || invoice?.supplierName || invoice?.supplier || 'Supplier')} />
...
function PaymentForm({
  ...
  const poOptions = useMemo(
    () =>
      purchaseOrders
        .filter((order) => {
          if (!formData.partyId) return true
          const selectedParty = parties.find((p) => String(p.value) === String(formData.partyId))
          const partyName = selectedParty?.label || ''
          const poSupplierId = String(order.supplierId || order.vendorId || '')
          const poSupplierName = String(order.supplier || order.supplierName || '')

          if (poSupplierId && String(formData.partyId)) {
            return poSupplierId === String(formData.partyId)
          }
          if (partyName && poSupplierName) {
            return poSupplierName.toLowerCase() === partyName.toLowerCase()
          }
          return true
        })
        .map((order) => {
          const itemsSum = Array.isArray(order.items)
            ? order.items.reduce((acc, item) => {
                const qty = Number(item.quantity || item.orderedQuantity || item.qty || 0)
                const price = Number(item.price || item.unitPrice || item.unitCost || item.cost || 0)
                const lineTot = Number(item.total || item.lineTotal || item.amount || qty * price || 0)
                return acc + lineTot
              }, 0)
            : 0
          ...
          const id = order.poId || order.id
          const poNum = order.poNumber || (id ? `PO-${id}` : 'PO')
          const suppName = order.supplier || order.supplierName || 'Supplier'
          return {
            value: id,
            label: `${poNum} - ${suppName} - ${formatCurrency(amt)}`,
            orderAmount: amt,
            supplierId: order.supplierId || order.vendorId || '',
          }
        }),
    [formData.partyId, purchaseOrders, parties],
  )
  ...
  function handleChange(event) {
    ...
    if (name === 'poId') {
      const selectedOption = poOptions.find((opt) => String(opt.value) === String(value))
      const selectedPo = purchaseOrders.find(
        (order) => String(order.poId || order.id) === String(value),
      )
      const poAmount = selectedOption?.orderAmount ?? (selectedPo
        ? selectedPo.totalAmount ?? selectedPo.grandTotal ?? selectedPo.amount ?? selectedPo.netAmount ?? ''
        : '')
      const supplierId = selectedOption?.supplierId || (selectedPo
        ? selectedPo.supplierId ?? selectedPo.vendorId ?? ''
        : '')

      setFormData((currentValue) => ({
        ...currentValue,
        poId: value,
        amount: poAmount !== '' && poAmount !== undefined && poAmount !== null ? String(poAmount) : currentValue.amount,
        partyId: supplierId ? String(supplierId) : currentValue.partyId,
      }))
      return
    }

    if (name === 'invoiceId') {
      const selectedInv = invoices.find(
        (inv) => getInvoiceId(inv) === String(value),
      )
      const invAmount = selectedInv
        ? selectedInv.balanceAmount ?? selectedInv.totalAmount ?? ''
        : ''

      setFormData((currentValue) => ({
        ...currentValue,
        invoiceId: value,
        amount: invAmount !== '' ? String(invAmount) : currentValue.amount,
      }))
      return
    }
...
export default function SupplierPaymentModule({
  suppliers,
  fetchPayments,
  createPayment,
  updatePayment,
  ...
  const subtitle = 'Track supplier disbursements, purchase-order payments, and reconciliation references.'
  const permissionKey = 'supplierPayments'
  const partyLabel = 'Supplier'
  const parties = suppliers

  const { hasPermission, user } = useAuth()
  const navigate = useNavigate()
  const isSupplier = true
  const defaultVisiblePaymentColumns = SUPPLIER_PAYMENT_BASE_COLUMNS
  const paymentColumnsStorageKey = PAYMENT_COLUMNS_STORAGE_KEYS.supplier
  ...
  const purchaseOrderById = useMemo(
    () => new Map(purchaseOrders.map((purchaseOrder) => [String(purchaseOrder.poId || purchaseOrder.id), purchaseOrder])),
    [purchaseOrders],
  )
  const invoiceById = useMemo(() => new Map(), [])
  ...
  const loadPayments = useCallback(async ({ updateLoading = true, isMounted = () => true } = {}) => {
    ...
    try {
      const [paymentsResponse, purchaseOrdersResponse] = await Promise.all([
        fetchPayments(),
        getPurchaseOrders(),
      ])
      ...
      if (purchaseOrdersResponse.success) {
        ...
        setError(getPaymentLoadErrorMessage(purchaseOrdersResponse.error || 'Unable to load purchase orders.'))
      }
    } catch (loadError) {
      if (isMounted()) {
        setPayments([])
  ...
  const handleExport = useCallback((rows = filteredPayments) => {
    ...
    exportPaymentsToPdf(exportRows, true, user)
  ...
  const handleDownloadReceipt = useCallback(async (payment) => {
    try {
      await downloadPaymentReceiptPdf({
        payment,
        purchaseOrder: purchaseOrderById.get(String(payment.poId)),
  ...
  }, [payments, purchaseOrderById, title, user])
  ...
  function handlePrint(rows = selectedPayments) {
    ...
    const receiptHtml = printRows.map((payment) => {
      const purchaseOrder = purchaseOrderById.get(String(payment.poId))
      const metrics = getSupplierReceiptAllocationMetrics(payment, purchaseOrder, payments)
      const remainingBalance = Number(metrics.outstandingAfter || 0)
      const purchaseOrderStatusLabel = formatPurchaseOrderStatus(purchaseOrder?.status || payment.poStatus)
        ...
        <section class="receipt-sheet">
          ...
          <div class="receipt-info-card">
            <div class="info-cell">
              ...
              <span class="info-value info-value--large">${escapeXml(formatPersonName(payment.partyName || purchaseOrder?.supplierName || purchaseOrder?.supplier) || 'Supplier')}</span>
            </div>
            <div class="info-cell">
              <span class="info-label">Purchase Order</span>
              <span class="info-value">${escapeXml(getPurchaseOrderNumber(payment, purchaseOrder))}</span>
            </div>
            <div class="info-cell">
              <span class="info-label">Order Date</span>
              <span class="info-value">${escapeXml(formatReceiptDate(purchaseOrder?.orderDate || payment.poDate || payment.paymentDate))}</span>
            </div>
            <div class="info-cell">
              <span class="info-label">PO Status</span>
              <span class="badge ${getPurchaseOrderReceiptStatusClass(purchaseOrderStatusLabel)}">${escapeXml(purchaseOrderStatusLabel)}</span>
            </div>
          </div>
    ...
    printWindow.document.write(`<!doctype html><html><head><title>Supplier Payment Receipt - IMS</title><style>
      ...
      render: (payment) => {
        ...
        if (!name || name === '-') {
          ...
          if (partyId && Array.isArray(parties)) {
            ...
            if (matched) {
              name = matched.name || matched.companyName || matched.company || matched.supplierName || matched.customerName || '-'
            }
          }
        }
  ...
  return (
    <div className="page payments-page payments-page--supplier">
      <header className="resource-center__inventory-header" aria-label="Supplier Payments summary">
      ...
      {hasFatalError ? (
        ...
        <div className={`${paymentTableCardClassName} resource-center__inventory-table-card`}>
          <DataTable
            ...
            searchPlaceholder="Search by Payment No, Supplier, Amount, or Method"
      ...
      {detailTarget ? (
        <PaymentDetailsDrawer
          ...
          purchaseOrder={purchaseOrderById.get(String(detailTarget.poId))}
          allPayments={payments}
          onClose={() => setDetailTarget(null)}
          onPrint={handlePrint}
          onDownloadReceipt={handleDownloadReceipt}
          onExport={handleExport}
          onOpenPurchaseOrder={(selectedPayment) => {
            const poId = selectedPayment?.poId
            navigate(poId ? `/inventory/purchases/${poId}` : '/inventory/purchases')
          }}
        />
      ) : null}
