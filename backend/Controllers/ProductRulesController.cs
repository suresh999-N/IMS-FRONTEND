...
{
    [ApiController]
    [Route("api/product-rules")]
    [Tags("SystemSettingsSection")]
    public class ProductRulesController : ControllerBase
    {
        private readonly AppDbContext _context;
        private const string SectionKey = "product_rules";

        public ProductRulesController(AppDbContext context)
        {
            _context = context;
        }
        ...
        [HttpGet]
        public async Task<IActionResult> GetProductRules()
        {
            ...
            if (section == null)
            {
                return NotFound("Product rules section not found.");
            }

            return Ok(section);
        }

        // PUT: api/product-rules
        [HttpPut]
        public async Task<IActionResult> UpdateProductRules([FromBody] List<UpdateSystemRuleDto> rules)
        {
        ...
        // POST: api/product-rules/reset
        [HttpPost("reset")]
        public async Task<IActionResult> ResetProductRules()
        {
            var section = await _context.SystemSettingSections
                .Include(x => x.Rules)
            ...
            return Ok(new
            {
                Message = "Product rules reset successfully."
            });
        }
    }
}