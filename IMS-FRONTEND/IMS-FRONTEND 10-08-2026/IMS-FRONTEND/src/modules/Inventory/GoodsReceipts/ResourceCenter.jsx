...
function getResourceFieldClassName(config, field) {
  if (config.key === 'goodsReceipts') {
    return `resource-form__field--${field.name}`
  }
  if (config.key !== 'subCategories') {
    return ''
  }
...
function getGoodsReceiptRemainingQuantity(poId, referenceData, receiptRows = []) {
  ...
  if (!Number.isFinite(orderedQuantity)) {
    const items = purchaseOrder.lineItems || purchaseOrder.items || purchaseOrder.products || purchaseOrder.orderItems || purchaseOrder.lines || []
    if (Array.isArray(items) && items.length > 0) {
      const pId = purchaseOrder.productId || purchaseOrder.ProductId
      const vId = purchaseOrder.variantId || purchaseOrder.VariantId
      ...
      if (!Number.isFinite(orderedQuantity)) {
        orderedQuantity = items.reduce((sum, line) => {
          let qty = 0
          for (const key of qtyKeys) {
            const val = line[key]
            if (val !== undefined && val !== null && val !== '') {
  ...
  if (!Number.isFinite(orderedQuantity)) {
    orderedQuantity = Number(getReferenceValue(purchaseOrder, 'quantity'))
  }
...
function ResourceForm({
  ...
  function updateGoodsReceiptFromPurchaseOrder(value) {
    ...
    let price = ''
    if (purchaseOrder) {
      const priceKeys = ['price', 'Price', 'unitPrice', 'UnitPrice', 'rate', 'Rate', 'cost', 'Cost', 'costPrice', 'CostPrice']
  ...
  function renderField(field) {
    ...
    if (field.type === 'searchableSelect') {
      let referenceRows = (
        Array.isArray(referenceData[field.optionsFrom])
          ? referenceData[field.optionsFrom]
          : []
      ).filter(item => {
        if (field.optionFilter) {
          return field.optionFilter(item, referenceData, rows)
        }
        return true
      })
      ...
      return (
        ...
        >
          <SearchableSelect
            ...
            name={field.name}
            label={field.label}
            icon={getResourceFieldIcon(config, field)}
            value={formData[field.name] ?? ''}
        ...
        {config.key === 'goodsReceipts' ? (
          ...
          <>
            ...
            {isSubCategoriesForm && mode === 'create' ? (
              <button
                type="button"
                className="button button-secondary resource-form__draft-button"
...
function ResourcePage({ config }) {
  ...
  const loadRows = useCallback(async function loadRows(options = {}) {
    ...
    const [response, metricResponse, ...referenceResponses] = await Promise.all([
      ...
      ...referenceEntries.map(([, endpoint]) => apiRequest(endpoint)),
    ])
  ...
  const baseColumns = isSubCategoriesPage
    ...
    : (config.columns ?? []).map((column, index) => ({
        ...
        render: (row) => formatCellValue(row, column),
          ...
          render: (row) => (
            <ActionButtons className="table-actions table-actions--nowrap resource-center__row-actions">
              <button
                ...
                title="View Details"
                aria-label={`View Details ${config.entityName}`}
              >
                <Eye size={16} />
                <span>View</span>
              </button>
  ...
  return (
    <div className={`resource-center__page resource-center__page--${config.key}`}>
      {isSubCategoriesPage ? (
      ...
      {!isSubCategoriesPage && !isInventoryCompactPage && !isNotificationsPage && !isInvoicesPage ? (
      ...
      <div className={`card ${isSubCategoriesPage ? 'resource-center__table-card' : ''} ${isInventoryCompactPage ? 'resource-center__inventory-table-card' : ''} ${isAuditLogsPage ? 'resource-center__audit-table-card' : ''} ${isNotificationsPage ? 'resource-center__notifications-table-card' : ''} ${isInvoicesPage ? 'resource-center__invoices-table-card' : ''}`}>
        <DataTable
          className={isSubCategoriesPage ? 'resource-center__subcategories-table' : isInventoryCompactPage ? 'resource-center__inventory-table' : isNotificationsPage ? 'resource-center__notifications-table' : isInvoicesPage ? 'resource-center__invoices-table' : ''}
          rows={isNotificationsPage ? filteredNotificationRows : rows}
          columns={columns}
          loading={isLoading}
          ...
          filterContent={isSubCategoriesPage ? subCategorySelectedToolbarContent : notificationFilterContent}
          toolbarContent={isSubCategoriesPage ? subCategoryToolbarContent : tableToolbarContent}
          columnStorageKey={isSubCategoriesPage ? 'ims.subCategories.visibleColumns.warehouseParity.v1' : isNotificationsPage ? 'ims.notifications.visibleColumns.v2' : isInvoicesPage ? 'ims.invoices.visibleColumns.v2' : ''}
      ...
      {isFormOpen ? (
        <FormModal
          ...
          subtitle={isSubCategoriesPage
            ? 'Maintain catalog hierarchy with compact, validated operational fields.'
            : 'The form tracks changed fields, validates inline, and handles backend validation errors.'}
          onClose={closeForm}
          className={isSubCategoriesPage ? 'form-modal--subCategories' : ''}
          dialogClassName={isSubCategoriesPage ? 'form-modal__dialog--subCategories' : ''}
          bodyClassName={isSubCategoriesPage ? 'form-modal__body--subCategories' : (config.key === 'goodsReceipts' ? 'form-modal__body--goodsReceipts' : '')}
        >
          <ResourceForm
        ...
        >
          <div className="purchase-details">
              ...
              <div className="purchase-details__grid">
                    ...
                    return (
                      <div className="purchase-details__item" key={col.key}>
                        ...
                        <span className="purchase-details__value">
                          {typeof displayVal === 'object' ? displayVal : <strong>{String(displayVal || 'N/A')}</strong>}
                        </span>
                      </div>
                    )
                  })}
                {config.fields?.map((f) => {
                  if (f.type === 'textarea' && viewingRecord[f.name]) {
                    return (
                      <div className="purchase-details__item purchase-details__item--full" key={f.name}>
                        <span className="purchase-details__label">{f.label}</span>
                        <span className="purchase-details__value purchase-details__value--notes">
            ...
            <div className="button-row purchase-details__footer">
              <button type="button" className="button button-primary" onClick={() => setViewingRecord(null)}>
                Close
              </button>
            </div>
          </div>
...
export default function ResourceCenter({ resourceKey, hubKey, resourceKeys, initialResourceKey }) {
  const keys = useMemo(() => {
  ...
  const configs = keys.map((key) => RESOURCE_CONFIGS[key]).filter(Boolean)
  const preferredKey = configs.some((config) => config.key === initialResourceKey)
    ? initialResourceKey
    : configs[0]?.key ?? ''
  const [activeKey, setActiveKey] = useState(preferredKey)

  useEffect(() => {
    if (initialResourceKey && configs.some((config) => config.key === initialResourceKey)) {
      setActiveKey(initialResourceKey)
      return
    }

    if (!configs.some((config) => config.key === activeKey)) {
      setActiveKey(configs[0]?.key ?? '')
    }
  }, [activeKey, configs, initialResourceKey])
  ...
  return (
    <div className="page resource-center">
      ...
      <ResourcePage key={activeConfig.key} config={activeConfig} />
    </div>
  )
}
