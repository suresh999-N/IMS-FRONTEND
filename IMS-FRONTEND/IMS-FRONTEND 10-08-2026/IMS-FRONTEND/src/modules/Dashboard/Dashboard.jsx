...
function monthKey(item) {
  let year = readNumber(item, 'year', 'Year')
  let month = readNumber(item, 'month', 'Month')

  if (!year || !month) {
    const rawDate = item?.date || item?.Date || item?.invoiceDate || item?.InvoiceDate || item?.month || item?.Month || item?.period || item?.Period || ''
...
function buildMonthlyTrend(monthlySales = [], monthlyPurchases = []) {
  ...
  safeSales.forEach((item) => {
    ...
    if (!key) return
    const salesAmt = readNumber(item, 'totalSales', 'TotalSales', 'sales', 'Sales', 'totalAmount', 'TotalAmount', 'amount', 'Amount', 'revenue', 'Revenue', 'total', 'Total')
    const existing = rows.get(key)
    rows.set(key, {
      month: monthLabel(key),
      sales: (existing?.sales ?? 0) + salesAmt,
  ...
  safePurchases.forEach((item) => {
    const key = monthKey(item)
    if (!key) return
    const purchasesAmt = readNumber(item, 'totalPurchases', 'TotalPurchases', 'purchases', 'Purchases', 'totalAmount', 'TotalAmount', 'amount', 'Amount', 'total', 'Total')
    const existing = rows.get(key)
    rows.set(key, {
      month: monthLabel(key),
      sales: existing?.sales ?? 0,
      purchases: (existing?.purchases ?? 0) + purchasesAmt,
    })
  })
