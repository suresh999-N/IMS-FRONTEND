...
export function normalizeDashboardPayload(responses) {
  ...
  return {
    ...
    monthlySales: (() => {
      const parsed = monthlySales.success
        ? (getResponseList(monthlySales, 'monthlySales').length
            ? getResponseList(monthlySales, 'monthlySales')
            : getResponseList(monthlySales, 'sales'))
        : []
      if (parsed.length > 0) return parsed
      const salesList = recentSales.success ? getResponseList(recentSales) : []
      if (!salesList.length) return []
      const map = new Map()
      salesList.forEach((item) => {
        const dStr = item.invoiceDate || item.InvoiceDate || item.createdAt || item.CreatedAt || item.date || item.Date
        if (!dStr) return
        ...
        const k = `${y}-${m}`
        const amt = number(item.totalAmount ?? item.TotalAmount ?? item.amount ?? item.Amount)
        const prev = map.get(k) || { year: y, month: m, totalSales: 0 }
        prev.totalSales += amt
        map.set(k, prev)
      })
      return Array.from(map.values())
    })(),
    monthlyPurchases: monthlyPurchases.success
      ? (getResponseList(monthlyPurchases, 'monthlyPurchases').length
          ? getResponseList(monthlyPurchases, 'monthlyPurchases')
          : getResponseList(monthlyPurchases, 'purchases'))
      : [],
