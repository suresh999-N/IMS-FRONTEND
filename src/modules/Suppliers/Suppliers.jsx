...
function buildSupplierProfile(supplier) {
  ...
  return {
    ...
    totalPurchaseAmount: supplier.totalPurchaseAmount ?? supplier.totalPurchases ?? null,
    outstandingPayable: supplier.outstandingPayable ?? supplier.outstandingAmount ?? null,
...
function printSuppliers(suppliers) {
  const rows = suppliers.map((supplier) => `
    <tr>
      ...
      <td>${escapeHtml(formatLastPurchase(supplier.lastPurchaseDate, formatDate))}</td>
    </tr>
  `).join('')
  ...
  printWindow.document.write(`<!doctype html><html><head><title>Suppliers</title><style>
    ...
    <h1>Suppliers</h1>
    <table>
      <thead><tr><th>Supplier</th><th>Status</th><th>Category</th><th>GST / PAN</th><th>Purchases</th><th>Outstanding</th><th>Last Purchase</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
  </body></html>`)
...
export default function Suppliers({
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      style: { width: '150px', minWidth: '150px' },
      headerStyle: { width: '150px', minWidth: '150px' },
      render: (supplier) => formatLastPurchase(supplier.lastPurchaseDate, formatDate),
    },
    {
      key: 'status',
      label: 'Status',
