...
function formatCompactPaymentCurrency(value) {
  ...
  if (absoluteAmount >= 10000000) {
    return `${sign}â‚¹${(absoluteAmount / 10000000).toFixed(1)} Cr`
  }

  if (absoluteAmount >= 100000) {
    return `${sign}â‚¹${(absoluteAmount / 100000).toFixed(1)} L`
  }
...
function getPurchaseOrderReceiptStatusClass(status) {
  ...
  if (['received', 'completed', 'paid', 'reconciled'].includes(normalizedStatus)) return 'badge-green'
  if (['ordered', 'pending', 'partial', 'partially received', 'partially_received'].includes(normalizedStatus)) return 'badge-amber'
  return 'badge-red'
}
...
function buildReceiptSvg({ payment, purchaseOrder, invoice, metrics, generatedBy, isSupplier = true }) {
  ...
  const partyName = payment.partyName || (isSupplier ? (purchaseOrder?.supplierName || 'Supplier') : 'Customer')
  ...
  return `
    <svg xmlns="http://www.w3.org/2000/svg" width="${PDF_CANVAS_WIDTH}" height="${PDF_CANVAS_HEIGHT}" viewBox="0 0 ${PDF_CANVAS_WIDTH} ${PDF_CANVAS_HEIGHT}">
      ...
      <text class="sig-lbl" x="211" y="830" text-anchor="middle">${isSupplier ? 'Settled By' : 'Customer Signature'}</text>
...
function PaymentDetailsDrawer({ payment, purchaseOrder, allPayments = [], onClose, onPrint, onDownloadReceipt, onOpenPurchaseOrder }) {
      ...
      <aside className="payment-drawer__panel">
        ...
        <section className="payment-drawer__summary-card" aria-label="Payment summary">
          ...
          <div className="payment-drawer__summary-customer">
            <span>Supplier</span>
            <strong>{activePayment.partyName || purchaseOrder?.supplierName || purchaseOrder?.supplier || 'Supplier'}</strong>
          </div>
        </section>
          ...
          <section className="payment-drawer__section payment-drawer__section--compact">
            <div className="payment-drawer__section-header">
              ...
              <span className="payment-drawer__invoice-actions">
                <button type="button" className="payment-drawer__invoice-link" onClick={() => onOpenPurchaseOrder?.(activePayment, purchaseOrder)}>
                  {purchaseOrderNumber}
                </button>
                <button
                  type="button"
            ...
            {status === 'Cancelled' ? (
              <li>
                ...
                <div>
                  ...
                  <p>{formatDate(payment.cancelledAt)} Â· {payment.cancellationReason || 'No reason recorded'}</p>
                </div>
              </li>
            ) : null}
          </ol>
        ...
        <section class="receipt-sheet">
          ...
          <div class="receipt-info-card">
            <div class="info-cell">
              ...
              <span class="info-value info-value--large">${escapeXml(payment.partyName || purchaseOrder?.supplierName || purchaseOrder?.supplier || 'Supplier')}</span>
            </div>
            <div class="info-cell">
