...
function exportBrandsCsv(items) {
  const headers = ['Brand Name', 'Description', 'Last Updated']
...
function printBrands(items) {
  const rows = items
    ...
    <tr>
      ...
      <td>${escapeHtml(formatReferenceDate(item.updatedAt || item.createdAt))}</td>
    </tr>
  `,
    )
    .join('')
  ...
  printWindow.document.write(`<!doctype html><html><head><title>Brands</title><style>
    ...
    <h1>Brands</h1>
    <table>
      <thead><tr><th>Brand</th><th>Description</th><th>Last Updated</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
  </body></html>`)
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        label: 'Last Updated',
        sortable: true,
        className: 'brands-col-date',
        tableWidth: 170,
        style: { width: 170, minWidth: 170 },
        headerStyle: { width: 170, minWidth: 170 },
        sortValue: (item) => new Date(item.updatedAt || item.createdAt || 0).getTime() || 0,
        render: (item) => formatReferenceDate(item.updatedAt || item.createdAt),
      },
      {
        key: 'actions',
        label: 'Actions',
      ...
      <div className="resource-center__page resource-center__page--brands">
        ...
        <div className="card resource-center__inventory-table-card">
          <DataTable
            ...
            defaultVisibleColumnKeys={['name', 'description', 'updatedAt', 'actions']}
            enableRowSelection
