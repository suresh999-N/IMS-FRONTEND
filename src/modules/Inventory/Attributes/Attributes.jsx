...
const ATTRIBUTE_DEFAULT_COLUMNS = ['name', 'values', 'actions']
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (item) => (
          <div className="attributes-table__values-chips">
            {item.values && item.values.length > 0 ? (
              item.values.map((val, idx) => (
                <span key={idx} className="badge badge--secondary badge--compact" title={val}>
                  {val}
                </span>
              ))
            ) : (
              <span className="text-muted text-xs">No values defined</span>
            )}
          </div>
        ),
      },
