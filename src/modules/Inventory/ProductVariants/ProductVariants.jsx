      ...
      const normalized = variantsList.map((variant) => {
        ...
        const mappedAttrs = varAttrList
          ...
          .map((va) => {
            ...
            if (attr && val) {
              return `${attr.name}: ${val.value || val.name}`
            }
            return ''
          })
          .filter(Boolean)
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        tableWidth: 100,
        className: 'variants-col-numeric',
        style: { width: 100, minWidth: 100, textAlign: 'right' },
        headerStyle: { width: 100, minWidth: 100, textAlign: 'right' },
        render: (item) => {
          const isLow = item.stock <= (item.reorderLevel || 0)
          return (
            <span className={isLow && item.stock > 0 ? 'text-warning font-semibold' : isLow ? 'text-danger font-semibold' : ''}>
              {item.stock}
            </span>
          )
        },
      },
      {
        ...
        render: (item) => (
          <StatusBadge type={String(item.status).toLowerCase() === 'inactive' ? 'critical' : 'active'}>
            {item.status || 'Active'}
          </StatusBadge>
        ),
      },
                ...
                <div className="resource-form__grid-2">
                  <div className="resource-form__field">
                    ...
                    <InputField
                      ...
                      onChange={(val) => setFormValues((prev) => ({ ...prev, variantName: val }))}
                  ...
                  <div className="resource-form__field">
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <InputField
                        ...
                        onChange={(val) => setFormValues((prev) => ({ ...prev, sku: val }))}
                ...
                <div className="resource-form__grid-2">
                  <div className="resource-form__field">
                    ...
                    <InputField
                      ...
                      onChange={(val) => setFormValues((prev) => ({ ...prev, priceDelta: val }))}
                  ...
                  {!editingItem ? (
                    <div className="resource-form__field">
                      ...
                      <InputField
                        ...
                        onChange={(val) => setFormValues((prev) => ({ ...prev, stockDelta: val }))}
                        placeholder="0"
                      />
