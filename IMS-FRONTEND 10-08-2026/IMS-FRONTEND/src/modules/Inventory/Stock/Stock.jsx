...
function ResourcePage({ config, navigationContent = null }) {
  ...
  const loadRows = useCallback(async function loadRows(options = {}) {
    ...
    if (!response.success) {
      ...
      setRows((response.data ?? []).map((row) => {
        ...
        if (config.key === 'stock') {
          return normalizeStockRow(normalizedRow)
        }

        if (config.key === 'stockMovements') {
          return normalizeStockMovement(normalizedRow)
        }
  ...
  useEffect(() => {
    ...
    function handleStockDataUpdated() {
      loadRows({ force: true })
    }
...
export default function Stock() {
  const resourceKey = 'stock';
  const hubKey = 'stockOperations';
