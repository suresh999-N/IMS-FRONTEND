  ...
  function renderField(field) {
    ...
    return (
      <InputField
        ...
        rows={field.type === 'textarea'
          ? (isSubCategoriesForm || config.key === 'stockAudits' ? 3 : 4)
          : undefined}
        value={formData[field.name]}
...
function ResourcePage({ config }) {
  ...
  const isStockAuditsPage = config.key === 'stockAudits'
    ...
    if (referenceEntries.length > 0) {
      setReferenceData(referenceEntries.reduce((result, [key], index) => {
        let list = referenceResponses[index]?.success
          ...
          : [];
        if (list.length === 0 && (key === 'products' || key === 'warehouses' || key === 'purchaseOrders')) {
          try {
            const rawData = localStorage.getItem('ims-frontend-data');
            if (rawData) {
              const parsed = JSON.parse(rawData);
              const storageKey = key === 'purchaseOrders' ? 'purchases' : key;
    ...
    <div className={`resource-center__page resource-center__page--${config.key}`}>
      ...
      {isFormOpen ? (
        <FormModal
          ...
          bodyClassName={isSubCategoriesPage
            ? 'form-modal__body--subCategories'
            : isStockAuditsPage
              ? 'form-modal__body--stockAudits'
              : ''}
        >
          <ResourceForm
