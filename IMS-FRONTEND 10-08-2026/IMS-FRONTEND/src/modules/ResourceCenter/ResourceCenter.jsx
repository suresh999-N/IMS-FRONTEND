...
import GoodsReceiptForm from '../Inventory/GoodsReceipts/components/GoodsReceiptForm'
...
function getReferenceOptionLabel(item, key, referenceData) {
  if (typeof key === 'function') {
    return key(item, referenceData)
  }
...
function getEmailOrPhoneError(value, label = 'Email or Phone Number') {
  const clean = String(value || '').trim()
  if (clean.includes('@')) {
    return getEmailError(clean, { required: false }) ? 'Enter a valid email address.' : ''
  }

  const cleanPhone = clean.replace(/[\s\-()]/g, '')
  const hasInvalidChars = /[^\d+]/.test(cleanPhone)
  const plusIndex = cleanPhone.indexOf('+')
  const hasPlusInWrongPlace = plusIndex > 0
  const digits = cleanPhone.replace(/\+/g, '')

  if (hasInvalidChars || hasPlusInWrongPlace || digits.length < 7 || digits.length > 15) {
...
function getFieldError(field, value, mode, context = {}) {
  ...
  if (field.name === 'confirmPassword') {
    const passwordVal = context.formData?.password || context.formData?.passwordHash || ''
    const confirmVal = value || ''
    if (mode === 'create' && !confirmVal) {
...
function ResourceForm({
  ...
  function renderField(field) {
    ...
    if (field.type === 'searchableSelect') {
      ...
      const options = (field.optionsFrom
        ? referenceRows.map((item) => ({
            ...
            label: getReferenceOptionLabel(item, field.optionLabel, referenceData),
          }))
        : field.options ?? []
      ).filter((option) =>
        option &&
      ...
      return (
        ...
        >
          <SearchableSelect
            ...
            showPlaceholder={field.showPlaceholder}
          />
          {isReferenceLoading ? (
    ...
    if (field.type === 'checkbox') {
      return (
        <div className={`field ${getResourceFieldClassName(config, field)}`.trim()} key={field.name}>
          <label htmlFor={`resource-${config.key}-${field.name}`} style={{ visibility: 'hidden' }}>Spacer</label>
          <label className="resource-form__checkbox" id={`resource-${config.key}-${field.name}`} style={{ display: 'inline-flex' }}>
            <input
              type="checkbox"
              name={field.name}
              ...
              onChange={handleChange}
              onBlur={handleBlur}
              style={field.name === 'isActive' ? { accentColor: 'var(--primary)', cursor: 'pointer' } : undefined}
            />
    ...
    return (
      <InputField
        ...
        autoComplete={
          field.type === 'password'
            ? 'new-password'
            : (field.type === 'email' || field.type === 'emailOrPhone')
            ? 'new-email'
    ...
    >
      <div className="resource-form__section">
        ...
        {fields.length === 0 ? (
          ...
          <div className={`form-grid ${config.key === 'notifications' ? 'form-grid--notifications' : ''}`}>
    ...
    if (referenceEntries.length > 0) {
      setReferenceData(referenceEntries.reduce((result, [key], index) => {
        let list = referenceResponses[index]?.success
          ...
          : [];
        if (list.length === 0 && (key === 'products' || key === 'warehouses' || key === 'purchaseOrders' || key === 'stockTransfers')) {
          try {
            const rawData = localStorage.getItem('ims-frontend-data');
            if (rawData) {
              const parsed = JSON.parse(rawData);
              const storageKey = key === 'purchaseOrders' ? 'purchases' : (key === 'stockTransfers' ? 'stockTransfers' : key);
              list = (parsed[storageKey] || []).map((row) => normalizeResourceRow(row, {}));
            }
          } catch (e) {
  ...
  const columns = isSubCategoriesPage
    ...
    : [
        ...baseColumns,
        {
          ...
          headerStyle: { width: 140, minWidth: 140, maxWidth: 140 },
          render: (row) => (
    ...
    <div className={`resource-center__page resource-center__page--${config.key} ${isProductStylePage ? 'resource-center__page--product-style products-page' : ''}`}>
      ...
      {isFormOpen ? (
        <FormModal
          ...
          subtitle={
            ['stockTransfers', 'stockTransferItems', 'stockAudits', 'stockAuditItems', 'goodsReceipts', 'notifications'].includes(config.key)
              ? null
              : isSubCategoriesPage
          ...
          className={
            config.key === 'goodsReceipts'
              ...
              : ['stockTransfers', 'stockTransferItems', 'stockAudits', 'stockAuditItems', 'notifications'].includes(config.key)
              ? `form-modal--${config.key}`
              : ''
          }
